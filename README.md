...
The primary goal of this project is to create a powerful, interactive search engine using Streamlit that utilizes YOLOv11 object detection metadata. The application aims to automate the process of analyzing large image directories and indexing them based on object counts and classes, enabling users to perform complex, quantitative queries such as "Find all images with 2-5 cars AND at least one person."

# Algorithm

The application operates in two primary phases: Data Ingestion (Inference/Loading) and Data Query (Search and Display).

# Phase 1: Data Ingestion

Model Initialization: The user provides the path to a trained YOLOv11 model (yolo11m.pt) and an image directory path.

Batch Processing (Inference): The YOLOv11Inference engine processes every image in the directory.

Metadata Extraction: For each image, the system records:

The image file path.

A dictionary of all detected classes and their total counts (class_counts).

A list of raw detections (class name, confidence, and bounding box coordinates).

Persistence: The complete metadata is saved to a single JSON file. Alternatively, a pre-existing metadata file can be loaded directly, skipping the inference steps.

# Phase 2: Data Query and Display

Parameter Definition: The user selects a search mode ('OR' or 'AND'), a list of target classes, and optional maximum count thresholds for each class.

Filtering Logic: The algorithm iterates through the metadata:

For each target class, it checks if the detection count meets the criteria (i.e., count >= 1 AND count <= threshold, or just count >= 1 if no threshold is set).

'OR' Mode: An image is included if any of the selected classes meet their criteria.

'AND' Mode: An image is included if all of the selected classes meet their criteria.

Visualization: Matching images are loaded, and the PIL library is used to dynamically draw bounding boxes and labels based on the user's display options (e.g., highlighting only matching classes).
...
# OUTPUT :
<img width="1918" height="1073" alt="image" src="https://github.com/user-attachments/assets/76fd13d7-0400-4628-97d3-21a8ec6ebe52" />
<img width="1918" height="1057" alt="image" src="https://github.com/user-attachments/assets/55a7e020-0a96-4732-a19c-ebd2db8c6b43" />
